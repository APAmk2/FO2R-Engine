...
public:
	...
	virtual void Set(asIScriptEngine *engine, void *value, int typeId);
	virtual void Set(asIScriptEngine *engine, const asINT64 &value);
	virtual void Set(asIScriptEngine *engine, const double &value);
	virtual void Set(asIScriptEngine *engine, CScriptDictValue &value);

	// Gets the stored value. Returns false if the value isn't compatible with the informed typeId
	virtual bool Get(asIScriptEngine *engine, void *value, int typeId) const;
	virtual bool Get(asIScriptEngine *engine, asINT64 &value) const;
	virtual bool Get(asIScriptEngine *engine, double &value) const;

	// Returns the address of the stored value for inspection
	virtual const void *GetAddressOfValue() const;

	// Returns the type id of the stored value
	virtual int  GetTypeId() const;

	// Free the stored value
	virtual void FreeValue(asIScriptEngine *engine);
...
public:
	...
	virtual CScriptDictionary &operator =(const CScriptDictionary &other);

	// Sets a key/value pair
	virtual void Set(const dictKey_t &key, void *value, int typeId);
	virtual void Set(const dictKey_t &key, const asINT64 &value);
	virtual void Set(const dictKey_t &key, const double &value);

	// Gets the stored value. Returns false if the value isn't compatible with the informed typeId
	virtual bool Get(const dictKey_t &key, void *value, int typeId) const;
	virtual bool Get(const dictKey_t &key, asINT64 &value) const;
	virtual bool Get(const dictKey_t &key, double &value) const;
	...
	virtual CScriptDictValue *operator[](const dictKey_t &key);
	virtual const CScriptDictValue *operator[](const dictKey_t &key) const;

	// Returns the type id of the stored value, or negative if it doesn't exist
	virtual int GetTypeId(const dictKey_t &key) const;

	// Returns true if the key is set
	virtual bool Exists(const dictKey_t &key) const;
	...
	// Deletes the key
	virtual bool Delete(const dictKey_t &key);

	// Deletes all keys
	virtual void DeleteAll();

	// Get an array of all keys
	virtual CScriptArray *GetKeys() const;
...
protected:
	...
	virtual void Init(asIScriptEngine *engine);
