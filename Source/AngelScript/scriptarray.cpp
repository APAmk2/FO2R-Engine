...
static void RegisterScriptArray_Native(asIScriptEngine *engine)
{
	...
	r = engine->RegisterObjectMethod("array<T>", "T &last()", asMETHODPR(CScriptArray, Last, (), void*), asCALL_THISCALL); assert(r >= 0);
	r = engine->RegisterObjectMethod("array<T>", "const T &last() const", asMETHODPR(CScriptArray, Last, () const, const void*), asCALL_THISCALL); assert(r >= 0);
	...
	r = engine->RegisterObjectMethod("array<T>", "void grow(uint)", asMETHODPR(CScriptArray, Grow, (asUINT), void), asCALL_THISCALL); assert(r >= 0);
...
{
	return At(IsEmpty() ? 0 : GetSize() - 1);
}

const void* CScriptArray::Last() const
{
	return At(IsEmpty() ? 0 : GetSize() - 1);
}
