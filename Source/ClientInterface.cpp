...
/************************************************************************/
    ...
    GmapVectY = 0;
    GmapMapCutOff.clear();
    SprMngr.PrepareSquare( GmapMapCutOff, Rect( 0, 0, GameOpt.ScreenWidth, GmapWMap.T ), COLOR_XRGB( 0, 0, 0 ) );
    SprMngr.PrepareSquare( GmapMapCutOff, Rect( 0, GmapWMap.T, GmapWMap.L, GmapWMap.B ), COLOR_XRGB( 0, 0, 0 ) );
    SprMngr.PrepareSquare( GmapMapCutOff, Rect( GmapWMap.R, GmapWMap.T, GameOpt.ScreenWidth, GmapWMap.B ), COLOR_XRGB( 0, 0, 0 ) );
    SprMngr.PrepareSquare( GmapMapCutOff, Rect( 0, GmapWMap.B, GameOpt.ScreenWidth, GameOpt.ScreenHeight ), COLOR_XRGB( 0, 0, 0 ) );
    ...
    if( GameOpt.HelpInfo )
    {
        ...
        if( GameOpt.DebugInfo )
        {
            ...
            SprMngr.DrawStr( Rect( 650, 5, 800, 300 ), Str::FormatBuf(
                                 ...
                                 GameOpt.FixedFPS, GameOpt.SoundVolume, GameOpt.MusicVolume
                                 ), FT_CENTERX, COLOR_XRGB( 255, 240, 0 ) );
        }

        SprMngr.DrawStr( Rect( 10, 10, GameOpt.ScreenWidth, GameOpt.ScreenHeight ), Str::FormatBuf(
                             ...
                             GameOpt.Ping, GameOpt.SoundVolume, GameOpt.MusicVolume
                             ), 0, COLOR_XRGB( 255, 248, 0 ), FONT_BIG );

        SprMngr.DrawStr( Rect( 0, 0, GameOpt.ScreenWidth, GameOpt.ScreenHeight ), MsgGame->GetStr( STR_GAME_HELP ), FT_CENTERX | FT_CENTERY, COLOR_TEXT_WHITE, FONT_DEFAULT );
    }
}
...
Rect FOClient::MessBoxCurRectScroll()
{
    ...
    if( GetActiveScreen() == SCREEN_NONE )
    {
        if( IsMainScreen( SCREEN_LOGIN ) )
            ...
            return Rect( 0, 0, GameOpt.ScreenWidth, 60 );
        else if( IsMainScreen( SCREEN_GLOBAL_MAP ) )
            return GmapWChat;
        else if( IsMainScreen( SCREEN_GAME ) && IntVisible && !IsScreenPresent( SCREEN__TOWN_VIEW ) )
        {
...
void FOClient::CurDraw()
{
    ...
    case CUR_USE_WEAPON:
    {
        ...
        SprMngr.Flush();
        SprMngr.DrawStr( Rect( GameOpt.MouseX + 6, GameOpt.MouseY + 6, x + 500, y + 500 ), str, FT_BORDERED, COLOR_TEXT_RED );
    }
    break;
    case CUR_USE_ITEM:
    case CUR_USE_SKILL:
...
{
    SprMngr.DrawSpriteSize( WaitPic, 0, 0, (float) GameOpt.ScreenWidth, (float) GameOpt.ScreenHeight, true, true );
    SprMngr.Flush();
}
