...
IniParser& IniParser::GetClientConfig()
{
    ...
    cfg_client.LoadFile( "data" DIR_SLASH_S "default.cfg", PT_ROOT );
    cfg_client.AppendToBegin( cfg_name, PT_ROOT);
    #else
    IniParser& cfg_mapper = GetMapperConfig();
    char       buf[ MAX_FOPATH ];
    cfg_mapper.GetStr( "ClientName", "FOnline", buf );
    Str::Append( buf, ".cfg" );
    cfg_client.LoadFile( ( GameOpt.ClientPath.c_std_str() + "data/default.cfg" ).c_str(), PT_ROOT );
    cfg_client.AppendToBegin( buf, PT_ROOT);
    #endif
