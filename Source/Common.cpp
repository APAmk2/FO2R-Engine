...
void GetClientOptions()
{
    ...
    GameOpt.DoubleClickTime = cfg.GetInt( "DoubleClickTime", COMBAT_MODE_ANY );
    ...
    GameOpt.Animation3dFPS = cfg.GetInt( "Animation3dFPS", 10 );
    GETOPTIONS_CMD_LINE_INT( GameOpt.Animation3dFPS, "-Animation3dFPS" );
    GETOPTIONS_CHECK( GameOpt.Animation3dFPS, 0, 1000, 10 );
    GameOpt.Animation3dSmoothTime = cfg.GetInt( "Animation3dSmoothTime", 0 );
    ...
    // Str
    cfg.GetStr( "FonlineDataPath", DIR_SLASH_SD "data", buf );
...
{
    label( GetWindowName() );
    position( ( Fl::w() - GameOpt.ScreenWidth ) / 2, ( Fl::h() - GameOpt.ScreenHeight ) / 2 );
    size( GameOpt.ScreenWidth, GameOpt.ScreenHeight );
