...
Sprite* Sprites_InsertSprite(Sprites* sprites, int draw_order, int hx, int hy, int cut, int x, int y, uint id, uint* id_ptr, short* ox, short* oy, uchar* alpha, Effect* effect, bool* callback )
{
	return &sprites->InsertSprite(draw_order, hx, hy, cut, x, y, id, id_ptr, ox, oy, alpha, &effect, callback);
}
